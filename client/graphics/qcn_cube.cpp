...
{  // this draws the necessary text for the cube view
 ...
 if (! bIsQCNLive) {
   ...
   if (!qcn_graphics::g_bFullScreen)  { // don't show the button press text in fullscreen/screensaver mode
        ...
        ttf_render_string(qcn_graphics::cfTextAlpha, 0, .32, 0, MSG_SIZE_NORMAL, yellow, TTF_HELVETICA, buf);
        sprintf(buf, "seismic sensor view");
        ttf_render_string(qcn_graphics::cfTextAlpha, 0, .30, 0, MSG_SIZE_NORMAL, yellow, TTF_HELVETICA, buf);
        ortho_done();
   }
   ...
   mode_unshaded();
   mode_ortho();
   ttf_render_string(qcn_graphics::cfTextAlpha, 0, .27, 0, MSG_SIZE_NORMAL, red, TTF_HELVETICA, "Red is Significance");
   ttf_render_string(qcn_graphics::cfTextAlpha, 0, .25, 0, MSG_SIZE_NORMAL, blue, TTF_HELVETICA, "Blue is Z-axis");
   ttf_render_string(qcn_graphics::cfTextAlpha, 0, .23, 0, MSG_SIZE_NORMAL, yellow, TTF_HELVETICA, "Yellow is Y-axis");
   ttf_render_string(qcn_graphics::cfTextAlpha, 0, .21, 0, MSG_SIZE_NORMAL, green, TTF_HELVETICA, "Green is X-axis");
   ttf_render_string(qcn_graphics::cfTextAlpha, 0, .19, 0, MSG_SIZE_NORMAL, cyan, TTF_HELVETICA, "Cyan is Magnitude");
   ttf_render_string(qcn_graphics::cfTextAlpha, 0, .17, 0, MSG_SIZE_NORMAL, magenta, TTF_HELVETICA, "Magenta is Variance");
   ortho_done();
 }
