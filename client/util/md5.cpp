...
void md5_from_file(const char* filename, char* checksum, int lCSLen) 
{
  memset(checksum, 0x00, lCSLen);

  FILE* fmd = boinc_fopen(filename, "rb");
  if (!fmd) {
	  fprintf(stderr, "Filename %s can't be opened\n", filename);
  }
  else {
    MD5 context(fmd);	
    strncpy(checksum, context.hex_digest(), lCSLen);
    fclose(fmd);
  }
}
...
void MD5::update(bfstream& stream){
...
MD5::MD5(unsigned char* string, unsigned int input_length)
{
  init();  // must be called be all constructors
  update(string, input_length);
  finalize ();
}
