...
Example usage:
./esc_trigmon -d 3 -sleep_interval 10 -time_interval 100
...
/* This function returns the distance (in km) between two points given as
   (lon1,lat1) & (lon2,lat2).This function written by Jesse Lawrence (April 2010) - 
    Contact: escawrence@stanford.edu   
*/
...
//    int j;
    ...
    for (int i=1; i<argc; i++) {
        if (!strcmp(argv[i], "-d")) {
            ...
            log_messages.printf(MSG_CRITICAL,
                "bad cmdline arg: %s\n\n"
                "Example usage: esc_trigmon -d 3 -sleep_interval 3 -count 10 -time_interval 10\n\n"
             , argv[i]
            );
            return 2;
        }
    ...
    log_messages.printf(MSG_NORMAL,
            "esc_trigmon started with the following options:\n"
            "  -time_interval   = %d\n" 
            "  -sleep_interval  = %f\n",
