...
/usr/local/mysql/bin/mysql -h db-private -u root -p1QCNQuake qcnalpha -e "UPDATE qcnalpha.qcn_constant SET value_int=unix_timestamp()-(3600*24*60) WHERE description='ArchiveTime'"
/usr/local/mysql/bin/mysql -h db-private -u root -p1QCNQuake qcnalpha < /home/boinc/archive.sql
# now that the archive is done, make a copy of it, so we don't have to dump this all out every night
/bin/rm -f $FILE_BACKUP
/bin/nice -n19 /usr/local/mysql/bin/mysqldump -h db-private -u root -p1QCNQuake --databases qcnarchive contarchive > $FILE_BACKUP
