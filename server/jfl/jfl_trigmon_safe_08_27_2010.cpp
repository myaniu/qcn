...
//    set_grid3D(g, e[1].elon, e[1].elat, e[1].edep, width, dx, zrange, dz);        // Set bounds of grid search
    ...
    g.yw = width/10;                               // Latitudinal width of grid
    g.xw = width/10;                               // Latitudinal width of grid
    g.zw = zrange/10;
...
/*  Check that event is not greater than 3 times father from the first station as the array is wide  */
   ...
   for (j = 1; j<=t[i].c_cnt; j++) {
    n = t[i].c_ind[j];
    t[n].dis = ang_dist_km(ln_x,lt_x,t[n].slon,t[n].slat);
...
void php_event_page(struct trigger t[], int i, struct event e[], char* epath) {  
   ...
   fprintf(fp11,"<p><table><tr><td>");
   fprintf(fp11,"<p><img src=\\\"./intensity.jpg\\\" width=\\\"400\\\"> \n");
   fprintf(fp11,"</td></tr><tr><td>");
   fprintf(fp11,"<p><a href=\\\"http://qcn.stanford.edu/images/ShakeMap_Scale.png\\\"><img src=\\\"http://qcn.stanford.edu/images/ShakeMap_Scale.png\\\" width=\\\"600\\\"></a> \n");
   ...
   time_t t_now; struct tm * t_now_gmt; time(&t_now);  t_now_gmt = gmtime(&t_now); // Current time
   fprintf(fp11,"<p>Page created on: %s \n",asctime(t_now_gmt));
...
//   fprintf(fp11,"<p><a href=\\\"http://qcn.stanford.edu/sensor/dl.php?&cbUseLat=1&LatMin=%f&LatMax=%f&LonMin=%f&LonMax=%date+start=%s\\\">Download data:</a>");
   ...
   fprintf(fp11,"\n\";\n");
   fprintf(fp11,"echo \\\"Page viewed on: \\\". date(\'M d Y\'). \\\" at \\\". date('h:i:s'); echo \\\" (UTC)\\\";");
...
void intensity_map_gmt(struct trigger t[], int i, struct event e[], char* epath){
   fprintf(stdout,"img: epath=%s",epath); 
   ...
   fprintf(fp10,"set PSFILE   = \"$OUTDIR/intensity.ps\" \n");                  // Set post script file name
...
/*  Cut topography */
   fprintf(fp10,"$GMT/grdcut $TOPO -G$GRDFILE $BOUNDS -V \n");                                                    // Cut grid topography file
   fprintf(fp10,"$GMT/grd2xyz $GRDFILE $BOUNDS -V > $TEMP \n");
   fprintf(fp10,"$GMT/surface -S2 $TEMP -G$GRDFILE $GRID $BOUNDS -V \n");                                           // Convert grid to xyz
   fprintf(fp10,"$GMT/grdgradient $GRDFILE -A270/20 -Ne0.5 -G$GRADFILE -V \n");                                // Create gradient shadow from topogray
...
/*  Plot Station Locations with color of measured intensity */
   fprintf(fp10,"$GMT/psxy $STATIONS $BOUNDS $PROJ $FLAGS3 -St0.25 -C$CPTFILE $B -W1p/0 >> $PSFILE \n"); 
...
/*  Execute GMT script  */
   char syscmd[sizeof "csh " + sizeof gmtfile]; sprintf(syscmd,"csh %s \n",gmtfile);
...
void preserve_dir(char * edir, char * epath, char * efile) {
   struct stat st;
   ...
   if(stat(epath,&st) == 0) {
     ...
     int i=0;
     for (i = 0; i<27; i++) {
       char epath2[sizeof epath + sizeof "/00000000_A"]; sprintf(epath2,"%s_%c",epath,ABC[i]);
       if (stat(epath,&st) != 0 ) {
...
/* Create an event directory                                  */
   ...
   char efile[sizeof epath + sizeof "/event.xy"]; sprintf(efile,"%s/event.xy",epath);
   char sfile[sizeof epath + sizeof "/stations.xyz"]; sprintf(sfile,"%s/stations.xyz",epath);
   char ifile[sizeof epath + sizeof "/intensity_map.xyz"]; sprintf(ifile,"%s/intensity_map.xyz",epath);
...
/* Create a file with the event location (lon,lat only)       */
   ...
   fprintf(fp10,"%f,%f,%f,%f\n",elon,elat,e[1].edep,e[1].e_mag);                         // Output event location
...
/* Create an interpolated intensity map:                      */
   ...
   intensity_map_gmt(t,i,e,epath); 
   php_event_page(t,i,e,epath); 
   ...
   for (i = 1; i<iCtr; i++) {                  // For each trigger 
    if (t[i].c_cnt > C_CNT_MIN) {              // If more than 4 correlated triggers, possible regional event
     ...
     if (t[i].c_cnt > e[1].e_cnt) {
      ...
      fprintf(stdout,"Longitude: %f  Latitude: %f  Depth: %f \n",e[1].elon,e[1].elat,e[1].edep);
      t_eq = (int) e[1].e_time;  t_eq_gmt = gmtime(&t_eq);  // Time of the Earthquake
      fprintf(stdout,"Quake Time: %s \n",asctime(t_eq_gmt));
