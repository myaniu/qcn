...
function ext_db_auth_activate() {
	...
	add_option('ext_db_authenticator_cookie','');    // CMC
}
...
function ext_db_auth_init(){
	...
	register_setting('ext_db_auth','ext_db_authenticator_cookie');  // CMC
}
...
?>
	...
	<?php settings_fields('ext_db_auth'); ?>
        ...
        <!-- CMC here - next tr section -->
        <tr valign="top">
            <th scope="row"><label>Authenticator Cookie Field</label></th>
			<td><input type="text" name="ext_db_authenticator_cookie" value="<?php echo get_option('ext_db_authenticator_cookie'); ?>" /></td>
              <td><span class="description">Use this if other parts of your domain are 'outside' of WordPress 
               but you want to save an authentication cookie to be used if successfully logged in via WordPress (and this ext_db plugin).  
              You will have a cookie named 'auth' that can be checked on the non-WordPress bits to facilitate with auto-login there etc.  
              If unused, leave this blank.</span></td>
        </tr>
      <!-- CMC here -- end of mods  -->
        <tr valign="top">
