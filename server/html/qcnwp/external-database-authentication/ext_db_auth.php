...
function ext_db_auth_check_login($username,$password) {
   ...
   $numrows = $numrows["numrows"];
    ...
    if ($numrows) {
        ...
        // CMC hack - add next line i.e. $sqlfields2['authenticator'] = "authenticator";
        $sqlfields['authenticator'] = "authenticator"; 
    ...
    // CMC hack - set cookies
   $auth = $extfields[$sqlfields['authenticator']];
   setcookie("auth",  $auth, time()+3600*24*365, "http://qcn.stanford.edu/sensor/");
   setcookie("auth",  $auth, time()+3600*24*365, "http://qcn.stanford.edu/continual/"); 
    // end CMC hack
