...
void handle_request(FILE* fin, FILE* fout, char* code_sign_key) {
    ...
    if (!p){
       // CMC here -- sreq has been parsed, see if contains a trigger
         ...
         log_messages.printf(MSG_DEBUG,
            "[HOST#%d] handle_request.C:: bTrigger = %s  iCount = %05d  iTrigger = %05d\n",
            sreq.hostid, bTrigger ? "true" : "false", iCount, iTrigger
         );

         // CMC end section

          process_request(sreq, sreply, code_sign_key, bTrigger);
          log_messages.set_indent_level(2);
