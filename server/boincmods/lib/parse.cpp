...
int dup_element(FILE* in, const char* tag_name, char** pp) {
	...
	long iMax; // max position/size of file
	char* strBig; // temp buffer for whole file
    long iRead; // number of bytes read to compare with iMax
    char *strStart, *strEnd; // start and end points of the tag in question

    sprintf(buf, "<%s>\n", tag_name);
    sprintf(end_tag, "</%s>", tag_name);

	iStart = ftell(in);  // save our current position

	if (retval=fseek(in, 0, SEEK_END)) return retval; // go to the end of the file
	iMax = ftell(in); // this is the max position of the file
	// go back to the beginning
	if (retval=fseek(in, 0, SEEK_SET)) return retval; // go to the start of the file

	strBig = (char*) malloc(iMax);
	...
	strStart = strstr(strBig, buf);
	strEnd = strstr(strBig, end_tag);

	if (!strStart || ! strEnd || strEnd < strStart) { // tags not found or out of place
		free(strBig); // don't forget to free our test buffer
		return ERR_XML_PARSE;  // tag not found
	}

	// now allocate the appropriate size, don't forget to add size of end tag
	int iReq = strEnd + strlen(end_tag) + 1 - strStart; 
	*pp = (char*) malloc(iReq); // note add one for the /0 at the end (the memset will take care of)
	memset(*pp, 0x00, sizeof(char) * iReq);
	// now copy over the appropriate bits
	memcpy(*pp, strStart, sizeof(char) * (iReq-1)); // copy over 1 minus the max, i.e. keep the /0 at the end

	// APP_INIT_DATA memory manage will take care of freeing *pp eventually
	// but now free the test string
	free(strBig);
    
	// don't forget to go to the end of this part of the file!
	fseek(in, iStart + iReq - 1, SEEK_SET);
	return 0;
...
#endif
const char *BOINC_RCSID_3f3de9eb18 = "$Id: parse.cpp 16449 2008-11-08 04:25:02Z davea $";
